...
print_bold() {
	echo -e "${BOLD}$1${NORMAL_TEXT}" | tee /dev/tty1 "$SYSTEM_MNT"/sysforge-wizard.log
}
print_bold_red() {
	echo -e "${BOLD_RED}$1${NORMAL_TEXT}" | tee /dev/tty1 "$SYSTEM_MNT"/sysforge-wizard.log
}
print_step() {
	echo -e "${BOLD_GREEN}$1${NORMAL_TEXT}" | tee /dev/tty1 "$SYSTEM_MNT"/sysforge-wizard.log
}
print_text() {
	echo -e "$1" | tee /dev/tty1 "$SYSTEM_MNT"/sysforge-wizard.log
}
exit_error() {
	print_text "$1"
	exit 1
...
if [ -s "$QUEUE" ]; then
	...
	. "$SYSTEM_BUILD_CONTROL_DIR/$NEXT_SCRIPT" 2>&1 | tee /dev/tty1 "$SYSTEM_MNT"/sysforge-wizard.log || exit_error "Error: while running $NEXT_SCRIPT"
